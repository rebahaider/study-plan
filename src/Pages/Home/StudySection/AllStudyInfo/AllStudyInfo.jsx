...
const AllStudyInfo = () => {
    ...
    const { isPending, data } = useQuery({
        ...
        queryFn: async () => {
            const res = await fetch(`https://assignment-12-server-one-flax.vercel.app/studyServices/${id}`);
            return res.json();
        }
            ...
            <div className="p-6 m-4 rounded-md shadow-md bg-gray-100 text-gray-900 mx-auto">
                <div className="mt-6 mb-6">
                    ...
                    <div className="mt-6">
                        {
                            disable ? (
                                <button className="btn btn-error" disabled>Registration Closed</button>
                            ) : (
                                <Link to={"/userDashboard/payment"} className="relative rounded px-5 py-2.5 overflow-hidden group bg-green-500 relative hover:bg-gradient-to-r hover:from-green-500 hover:to-green-400 text-white hover:ring-2 hover:ring-offset-2 hover:ring-green-400 transition-all ease-out duration-300">
